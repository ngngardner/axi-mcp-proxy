...
fn make_config(upstreams: Vec<(&str, SocketAddr)>, tools: HashMap<String, ToolConfig>) -> Config {
...
async fn call_proxy_tool(addr: SocketAddr, tool: &str, args: Value) -> (String, bool) {
...
fn mock_filterable() -> MockServer {
    MockServer::new("mock-filterable").add_tool(
        "list_tasks",
        "List tasks with mixed states",
        |_| {
            text_result(
                r#"[
...
async fn test_repo_context() {
    ...
    let (text, _) = call_proxy_tool(
        proxy_addr,
        "repo_context",
        json!({"owner": "testorg", "repo": "testrepo"}),
    )
    .await;
        ...
        ToolConfig {
            ...
            steps: vec![
                ...
                StepConfig {
                    name: "builds".into(),
                    upstream: "ci".into(),
                    tool: "list_builds".into(),
                    args: HashMap::from([("repo".into(), json!("$param.repo"))]),
                    depends_on: vec![],
                    transform: Some(TransformConfig {
                        pick: Some(vec!["id".into(), "status".into(), "branch".into()]),
                        rename: None,
                        filter: None,
                        truncate: None,
                        parsed_filter: None,
            ...
            next_steps: vec![NextStepConfig {
                command: "build_details <id>".into(),
                description: "Build details".into(),
                when: None,
            }],
            ...
            steps: vec![StepConfig {
                ...
                transform: Some(TransformConfig {
                    ...
                    rename: Some(HashMap::from([
                        ("number".into(), "pr_number".into()),
                        ("author".into(), "created_by".into()),
                    ])),
                    filter: None,
                    truncate: None,
                    parsed_filter: None,
    ...
    let tools = HashMap::from([(
        ...
        ToolConfig {
            ...
            steps: vec![
                ...
                StepConfig {
                    name: "fetch_details".into(),
                    upstream: "slow".into(),
                    tool: "get_details".into(),
                    args: HashMap::from([("id".into(), json!("$step.fetch_id.id"))]),
                    depends_on: vec!["fetch_id".into()],
                    transform: None,
                },
            ],
            output_fields: vec![],
...
async fn test_sum_aggregate() {
    ...
    let tools = HashMap::from([(
        ...
        ToolConfig {
            ...
            steps: vec![StepConfig {
                name: "scores".into(),
                upstream: "numeric".into(),
                tool: "get_scores".into(),
                args: HashMap::new(),
                depends_on: vec![],
                transform: None,
            }],
